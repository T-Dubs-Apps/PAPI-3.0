...
def get_audio_bytes(text):
    try:
        ...
        fp = io.BytesIO()
        tts.write_to_fp(fp)
        fp.seek(0) # CRITICAL FIX: Reset stream position for playback
        return fp
    except Exception as e: return None

def guardian_scan(text):
    try:
        if profanity.contains_profanity(text): return False, "⚠️ Please use kind words."
        if TextBlob(text).sentiment.polarity < -0.3: return True, "intervention"
        return True, "clean"
    except: return True, "clean"

def main():
    with st.sidebar:
        st.title("⚙️ Cloud Control")
    ...
    st.header(f"{st.session_state['assistant_name']} (Cloud Edition)")
    
    if st.session_state['user_tier']:
        for chat in st.session_state['chat_history']:
            with st.chat_message(chat["role"]): 
                st.write(chat["msg"])
                if "audio" in chat and chat["audio"]: st.audio(chat["audio"], format='audio/mp3')
        ...
        if txt:
            ...
            if not safe:
                ...
                reply = f"[{st.session_state['assistant_name']}]: Processed '{txt}'"
                audio = get_audio_bytes(reply)
